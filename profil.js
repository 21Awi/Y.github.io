...
let beetwen_mid = document.querySelector('#beetwen_mid')
window.addEventListener('load', (event) =>{
    profiles.style.backgroundImage='url(' + chooser_photos() + ')';
    beetwen_mid.style.backgroundImage= 'url(' + chooser_photos() + ')';
})
...
//Nick pisanie 
let Nick_under = document.querySelector('#Nick_under_profil')
...
window.addEventListener('load', (event) =>{
    ...
    Nick_under.innerHTML = localStorage.getItem("Nazwa").slice(1,localStorage.getItem("Nazwa".length)-1)
})
